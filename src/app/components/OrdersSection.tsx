...
export default function OrdersSection({
  ...
  const onlineDrivers = drivers.filter(
    (d) => d.status === "Online" && d.verificationStatus === "Verified",
  );
        ...
        nextStatus === "Accepted"
          ...
          : nextStatus === "Preparing"
            ...
            : nextStatus === "OutForDelivery"
              ? `Dispatched out with rider ${driverToAssign?.name || order.driverName || "fleet"}.`
              : nextStatus === "Delivered"
                ? "Rider confirmed delivery completed successfully."
  ...
  const lanes = [
    {
      title: "Incoming",
      statusKey: ["Pending"] as Order["status"][],
      color: "border-amber-400",
      badgeCount: orders.filter((o) => o.status === "Pending").length,
    },
    {
      title: "Cooking",
      statusKey: ["Accepted", "Preparing"] as Order["status"][],
    ...
    },
    {
      ...
      statusKey: ["OutForDelivery"] as Order["status"][],
      color: "border-blue-500",
      badgeCount: orders.filter((o) => o.status === "OutForDelivery").length,
    },
    {
      title: "Done",
      statusKey: ["Delivered", "Cancelled"] as Order["status"][],
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            {lanes.map((col, idx) => {
              const colOrders = orders.filter((o) =>
                col.statusKey.includes(o.status),
              );
              return (
                <div
                  key={idx}
                ...
                >
                  {/* Column header */}
                  <div
                    className={`p-3 border-b-2 ${col.color} bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl flex justify-between items-center shadow-sm`}
                  >
                    <span className="text-xs font-bold text-zinc-900 dark:text-zinc-100">
                      {col.title}
                    </span>
                    <span className="bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 text-[10px] font-black rounded text-zinc-500 dark:text-zinc-400">
                      {colOrders.length}
                    </span>
                  </div>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
                ...
                <div className="flex gap-2 w-full sm:w-auto shrink-0 flex-wrap">
                  {selectedOrder.status === "Pending" && (
                    <>
                      <button
                        onClick={() =>
                          handleProgressOrder(selectedOrder, "Accepted")
                        }
                  ...
                  {selectedOrder.status === "Accepted" && (
                    <button
                      onClick={() =>
                        handleProgressOrder(selectedOrder, "Preparing")
                      }
                  ...
                  {selectedOrder.status === "OutForDelivery" && (
                    <button
                      onClick={() =>
                        handleProgressOrder(selectedOrder, "Delivered")
                      }
                      className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-[10px] px-3.5 py-2 rounded-lg flex items-center gap-1 transition-all"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-800/20 text-xs space-y-2.5 text-zinc-500 dark:text-zinc-400">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-zinc-800 dark:text-zinc-200">
                        ${selectedOrder.serviceFee.toFixed(2)}
                      </span>
                    </div>
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">
                    Logistic Dispatch Participants
                  </h4>

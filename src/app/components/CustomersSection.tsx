    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {filteredCustomers.length === 0 ? (
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-12 text-center rounded-2xl flex flex-col items-center justify-center">
          ...
          <p className="text-sm font-semibold text-zinc-800 dark:text-zinc-200">No customers found</p>
            ...
            >
              ...
              <div className="mt-5 pt-3.5 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center gap-3">
                <div className="flex gap-4">
                  <div>
                    <p className="text-[9px] font-bold text-zinc-400 uppercase">Spend</p>
                    <p className="text-xs font-black text-zinc-900 dark:text-white mt-0.5">${cust.totalSpent.toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-[9px] font-bold text-zinc-400 uppercase">Orders</p>
                    <p className="text-xs font-black text-zinc-900 dark:text-white mt-0.5">{cust.ordersCount}</p>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-150">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150">
            ...
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900/40">
              <div className="flex items-center gap-3">
                <span className="text-3xl p-1 bg-white dark:bg-zinc-800 rounded-xl shadow-sm">{selectedCust.avatar}</span>
                <div>
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white">{selectedCust.name}</h3>
                  <p className="text-[10px] text-zinc-400">Account Registry ID: {selectedCust.id}</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Customer Profile</h4>
                  ...
                  <div className="space-y-2.5 text-xs text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-800/40 p-4 rounded-xl border border-zinc-100 dark:border-zinc-800">
                    <div className="flex justify-between">
                      <span className="font-semibold text-zinc-400">Email Address</span>
                      <span className="font-bold text-zinc-800 dark:text-zinc-200">{selectedCust.email}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-semibold text-zinc-400">Phone Contact</span>
                      <span className="font-bold text-zinc-800 dark:text-zinc-200">{selectedCust.phone}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-semibold text-zinc-400">Registration Date</span>
                      <span className="font-bold text-zinc-800 dark:text-zinc-200">{selectedCust.joinedDate}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-semibold text-zinc-400">Rating Status</span>
                      <span className={`font-bold px-2 py-0.5 rounded text-[10px] ${
                        selectedCust.status === 'Active' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'
                      }`}>
                        {selectedCust.status}
                      </span>
                    </div>
                ...
                {/* Addresses */}
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider">Registered Locations</h4>
                  <div className="space-y-2">
                    {selectedCust.addresses.map((address, idx) => (
                      <div key={idx} className="flex gap-2.5 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800/80 text-[11px] text-zinc-500 dark:text-zinc-400 bg-zinc-50/50 dark:bg-zinc-900/30">
              ...
              <div className="space-y-4 border-t border-zinc-100 dark:border-zinc-800 pt-5">
                <h4 className="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                  <History className="w-4 h-4 text-orange-500" /> Order History Log ({customerOrders.length} transactions)
                </h4>

                {customerOrders.length === 0 ? (
                  <p className="text-xs text-zinc-400 italic">No past orders found for this customer.</p>
                ) : (
                  <div className="space-y-3">
                    {customerOrders.map((order) => (
                      <div key={order.id} className="flex items-center justify-between p-3.5 rounded-xl border border-zinc-100 dark:border-zinc-800/80 bg-zinc-50/50 dark:bg-zinc-900/40">
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-bold text-zinc-900 dark:text-white">{order.id}</span>
                            <span className="text-[10px] text-zinc-400">•</span>
                            <span className="text-[10px] font-semibold text-orange-500">{order.restaurantName}</span>
                          </div>
                          <p className="text-[10px] text-zinc-400 mt-1">{new Date(order.createdAt).toLocaleDateString()} • {order.items.reduce((s,i)=>s+i.quantity,0)} items</p>
                        </div>
                        
                        <div className="text-right">
                          <span className="text-xs font-black text-zinc-900 dark:text-white">${order.total.toFixed(2)}</span>
                          <div className="mt-1">

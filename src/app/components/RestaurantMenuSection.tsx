...
export default function RestaurantMenuSection({
  ...
  const [openAiApiKey, setOpenAiApiKey] = useState(() => {
    if (typeof window !== "undefined") {
      return window.localStorage.getItem("nowlny_openai_key") || "";
    }
    return "";
  });

  const handleUpdateOpenAiKey = (key: string) => {
    setOpenAiApiKey(key);
    if (typeof window !== "undefined") {
      window.localStorage.setItem("nowlny_openai_key", key);
    }
  };
  ...
  // What is in flight is held in a ref *and* in state. The ref is what the drag
  // handlers read: `dragstart` is not a discrete event, so a state update made
  // there is not guaranteed to have rendered before the first `dragover`
  // arrives — and a `dragover` that reads `null` never calls preventDefault(),
  // which tells the browser this is not a drop zone. The state copy exists only
  // to paint the drag/drop highlighting.
  ...
  /** True when the row under the pointer can accept what is being dragged. */
  const sectionAccepts = (source: DragRef | null, sectionId: string) => {
    if (!source) return false;
    if (source.kind === "section")
      return canSortSections && source.id !== sectionId;
  ...
  // Handlers ask the ref (current), renders ask the state (reactive).
  const sectionAcceptsDrag = (sectionId: string) =>
    sectionAccepts(draggingRef.current, sectionId);
  const itemAcceptsDrag = (itemId: string) =>
    itemAccepts(draggingRef.current, itemId);
                  ...
                  <>
                    <div className="space-y-1">
                      <label
                        ...
                        className="text-[9px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest block"
                      >
                        {t("rmenu.openai_key_label")}
                      </label>
                      <input
                        id="openai-api-key"
                        type="password"
                        placeholder={t("rmenu.openai_key_placeholder")}
                        value={openAiApiKey}
                        onChange={(e) => handleUpdateOpenAiKey(e.target.value)}
                        className="w-full bg-white dark:bg-zinc-900 border border-zinc-250 dark:border-zinc-850 text-[11px] font-bold text-zinc-850 dark:text-zinc-100 placeholder-zinc-400 rounded-xl px-3 py-2.5 focus:outline-none focus:ring-1 focus:ring-purple-500 shadow-sm"
                      />
                    </div>
                    <p className="text-[9px] text-zinc-400 leading-normal">
                      💡 <strong>{t("rmenu.safe_secure")}</strong>:{" "}
                      {t("rmenu.openai_key_hint")}{" "}
                      <a
            ...
            {sections
              ...
              .map((sec) => {
                ...
                const isSectionDragged =
                  dragging?.kind === "section" && dragging.id === sec.id;
                const isSectionDropTarget =
                  dropTargetId === sec.id && sectionAccepts(dragging, sec.id);
                    ...
                    <div className="flex items-center justify-between pb-2 border-b border-zinc-200 dark:border-zinc-800">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <DragHandle
                          ...
                          onDragStart={(e) =>
                            startDrag(e, { kind: "section", id: sec.id })
                          }
                          onDragEnd={clearDrag}
                          onKeyDown={(e) => handleSectionKeys(e, sec.id)}
                        />
                        <h4 className="font-black text-sm text-zinc-900 dark:text-white uppercase tracking-wider truncate">
                          {sec.name}
                        </h4>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {secItems.map((item) => {
                          ...
                          const isItemDropTarget =
                            dropTargetId === item.id && itemAccepts(dragging, item.id);

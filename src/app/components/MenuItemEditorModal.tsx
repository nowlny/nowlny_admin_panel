...
export default function MenuItemEditorModal({
  ...
  const loadOptionGroups = async (menuItemId: string) => {
    ...
    try {
      ...
      setOptionGroups([]);
      setOptionsError(err?.message || "Couldn't load the add-on groups for this dish.");
    } finally {
  ...
  const handleDeleteGroup = async (group: MenuOptionGroup) => {
    ...
    const confirmed = await confirm({
      ...
      description:
        choices > 0
          ...
          : "This add-on group will be removed from this dish.",
    ...
    try {
      ...
      toast.error(err?.message || "Failed to delete option group");
    } finally {
  ...
  const handleSaveOption = async (groupId: string) => {
    if (!optionName.trim() || !optionPrice) {
      toast.error("Both an option name and a surcharge are required");
      return;
    }
  ...
  const handleDeleteOption = async (option: { id: string; name: string }) => {
    ...
    const confirmed = await confirm({
      ...
      description: "Customers will no longer be able to pick it for this dish.",
  ...
  return (
    <Modal
      ...
      description={item ? item.name : "Add a new dish to your store menu."}
      icon={
      ...
      {activeTab === "basic" && (
        ...
        >
          <div className="space-y-1.5">
            <label htmlFor={`${fieldId}-name`} className={labelClass}>Dish Title *</label>
            <input id={`${fieldId}-name`} type="text" required placeholder="e.g. Classic Cheeseburger" value={name} onChange={(e) => setName(e.target.value)} className={inputClass} />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label htmlFor={`${fieldId}-price`} className={labelClass}>Price ($) *</label>
            ...
            <div className="space-y-1.5">
              <label htmlFor={`${fieldId}-discounted-price`} className={labelClass}>Discounted Price ($)</label>
              <input id={`${fieldId}-discounted-price`} type="number" step="0.01" min="0" inputMode="decimal" placeholder="Optional" value={discountedPrice} onChange={(e) => setDiscountedPrice(e.target.value)} className={inputClass} />
            </div>
          </div>

          <div className="space-y-1.5">
            <label htmlFor={`${fieldId}-image`} className={labelClass}>Image URL</label>
            <input id={`${fieldId}-image`} type="text" placeholder="https://..." value={image} onChange={(e) => setImage(e.target.value)} className={inputClass} />
          </div>

          <div className="space-y-1.5">
            <label htmlFor={`${fieldId}-section`} className={labelClass}>Category Section *</label>
          ...
          <div className="flex flex-col gap-2 pt-2">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor={`${fieldId}-available`} className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Item is available (in stock)</label>
            </div>
            <div className="flex items-center gap-2">
              ...
              <label htmlFor={`${fieldId}-active`} className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Item is active (shown on menu)</label>
            </div>
            <div className="flex items-center gap-2">
              ...
              <label htmlFor={`${fieldId}-popular`} className="text-xs font-bold text-zinc-700 dark:text-zinc-300">Mark as popular</label>
            </div>
          </div>
        </form>
      )}
            ...
            <>
                  ...
                  <div className="p-4 space-y-3 bg-white dark:bg-zinc-900">
                    ...
                    {addingOptionToGroup === group.id ? (
                      <div className="flex gap-2 items-center mt-2 p-2 bg-zinc-50 dark:bg-zinc-800/30 rounded-lg border border-zinc-200 dark:border-zinc-700">
                        <input type="text" aria-label="Option choice name" placeholder="Option name" value={optionName} onChange={e => setOptionName(e.target.value)} className="flex-1 text-xs px-2 py-1.5 rounded bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700" />
                        <input type="number" step="0.01" min="0" inputMode="decimal" aria-label="Option choice surcharge" placeholder="Price" value={optionPrice} onChange={e => setOptionPrice(e.target.value)} className="w-20 text-xs px-2 py-1.5 rounded bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700" />
                        <button type="button" aria-label="Save option choice" disabled={isSavingOption} onClick={() => handleSaveOption(group.id)} className="p-2 bg-orange-500 text-white rounded hover:bg-orange-600 disabled:opacity-50">
                          {isSavingOption ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
                        </button>
                        <button type="button" aria-label="Cancel adding an option choice" onClick={resetOptionEditor} className="p-2 text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-white"><X className="w-3.5 h-3.5" /></button>
                      </div>
                    ) : (
                      <button type="button" onClick={() => setAddingOptionToGroup(group.id)} className="text-[10px] font-bold text-orange-500 flex items-center gap-1 hover:underline mt-2 ml-2">
              ...
              <div className="p-4 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-2xl bg-zinc-50 dark:bg-zinc-900/30">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                  <div className="space-y-1.5">
                    <label htmlFor={`${fieldId}-group-name`} className={labelClass}>Group Name</label>
                    <input id={`${fieldId}-group-name`} type="text" placeholder="e.g. Size, Toppings" value={groupName} onChange={e => setGroupName(e.target.value)} className="text-xs px-3 py-2 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full" />
                  </div>
                  <div className="space-y-1.5">
                    <label htmlFor={`${fieldId}-group-type`} className={labelClass}>Selection Type</label>
                    <div className="flex items-center gap-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 py-2">
                      <select id={`${fieldId}-group-type`} value={groupType} onChange={e => setGroupType(e.target.value as any)} className="bg-transparent focus:outline-none text-xs text-zinc-800 dark:text-zinc-200 flex-1 border-r border-zinc-200 dark:border-zinc-700 pr-2">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <label htmlFor={`${fieldId}-group-required`} className="text-xs font-semibold text-zinc-600 dark:text-zinc-400">Required?</label>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="flex justify-end gap-2">
                  {editingGroup && <button type="button" onClick={resetGroupEditor} className="px-3 py-1.5 text-xs font-bold text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-lg">Cancel</button>}
